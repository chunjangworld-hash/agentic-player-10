...
- **측정 구간 (2026-06-23 PlayMCP 디스코드 공식 답변)**: HTTP 요청 수신 ~ JSON-RPC 응답 송출 **end-to-end 전체**
- → LLM 호출을 우리 MCP 서버 내에서 하지 말고, 데이터 조회/포맷팅 중심으로 설계
- → 무거운 추론은 Kakao Tools의 호출 에이전트(GPT/Claude)가 처리하도록 위임
- → **외부 검색은 Tavily 우선(5분 캐싱) + 네이버는 1~2개 Tool에 한정** (평균 100ms 맞추기 위해)

## 외부 API 호출 (2026-06-23 PlayMCP 디스코드 공식 답변)

- PlayMCP 측 **별도 제한 없음**. 약관·저작권·개인정보 책임은 **100% 제작자**
- 우리는 다음 가드레일로 통과:
  - 네이버: `cache_ttl=0` + `mandatory_footer` 자동 부착
  - Tavily: 5분 캐싱 (약관 허용 범위)
  - 응답은 raw 미반환 — `ResponseBuilder`로 정제된 마크다운만
  - 개인정보 미저장 — Stateless 설계
