...
def check_suspicious_message(inp: CheckSuspiciousMessageInput) -> str:
    ...
    sections.append((
        "## ✏️ 호출 에이전트 다음 액션 지침\n"
        "**사용자 응답 톤**\n"
        f"- 위험도 [{verdict}]에 맞춰 침착하고 명확하게 설명\n"
        "- '100% 사기' '확실히 안전' 같은 단정 표현 ❌. '의심 신호가 있어요' / "
        "'주의가 필요해요' 같이 자녀가 안심할 만한 어조\n"
        "- 메타 해설 ❌. 도구 이름(check_suspicious_message 등)·JSON 노출 ❌\n\n"
        "**위험도가 '중간' 이상이면 — 사용자에게 옵션 카드 제시**\n"
        "'다음 중 어떤 도움이 필요하세요?\n"
        " 📨 부모님께 보낼 경고 카드 만들기\n"
        " 💾 결과를 나챗방에 저장\n"
        " 🔍 다른 메시지도 검토하기'\n"
        "→ 사용자 선택에 따라 compose_parent_warning / save_to_memo_chat Tool 호출\n\n"
        "**위험도가 '낮음' 또는 '없음'이면**\n"
        "안심시키되 일반 예방 한 줄: '큰 위험 신호는 없네요. 그래도 부모님이 링크는 클릭하지 마시고, "
        "의심되면 1577-0001(카드사) / 118(KISA) 직접 확인해보세요'",
        3,
    ))
